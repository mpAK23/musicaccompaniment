...
class CustomRecorderSampler {
    constructor(ac, destination, baseUrl, instrumentType) {
        this.ac = ac;
        this.destination = destination;
        this.baseUrl = baseUrl;
        this.instrumentType = instrumentType;
        this.semitoneOffset = 0;
        if (instrumentType === 'soprano-recorder') this.semitoneOffset = 12;
        else if (instrumentType === 'alto-recorder') this.semitoneOffset = 5;
        else if (instrumentType === 'tenor-recorder') this.semitoneOffset = 0;
        else if (instrumentType === 'bass-recorder') this.semitoneOffset = -7;
        this.sampleMap = {
            "C4": "C4.mp3",
            "E4": "E4.mp3",
            "A4": "A4.mp3",
        ...
        this.audioBuffers = {};
        this.activeNodes = [];
        this.loadPromise = this.initAudio();
    }
    ...
    async initAudio() {
        const promises = Object.keys(this.sampleMap).map(async note => {
            try {
                const response = await fetch(this.baseUrl + this.sampleMap[note]);
                const arrayBuffer = await response.arrayBuffer();
                this.audioBuffers[note] = await this.ac.decodeAudioData(arrayBuffer);
            } catch (e) {
                console.error("Custom sampler failed to load buffer:", note, e);
            }
        });
    ...
    start({ note, time, duration }) {
        ...
        const noteToMidi = { "C4": 60, "E4": 64, "A4": 69, "C5": 72 };
        
        const shiftedNote = note + this.semitoneOffset;

        let nearest = "C4";
        let minDiff = Infinity;
        for (let s in noteToMidi) {
            let diff = Math.abs(shiftedNote - noteToMidi[s]);
            if (diff < minDiff) {
                minDiff = diff;
                nearest = s;
            }
        ...
        const buffer = this.audioBuffers[nearest];
        if (!buffer) return;

        const sampleMidi = noteToMidi[nearest];
        const rate = Math.pow(2, (shiftedNote - sampleMidi) / 12);
...
async function prepareAudio() {
    ...
    if (!sfInstrument) {
        ...
        try {
            if (selected.includes('recorder')) {
                const baseUrl = "https://gleitz.github.io/midi-js-soundfonts/FluidR3_GM/recorder-mp3/";
                sfInstrument = new CustomRecorderSampler(ac, masterGain, baseUrl, selected);
            } else {
                let sfConfig = { 
                    instrument: selected,
                    destination: masterGain
                };
                sfInstrument = new Soundfont(ac, sfConfig);
            }
