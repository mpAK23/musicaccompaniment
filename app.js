...
class CustomRecorderSampler {
    constructor(ac, destination, baseUrl, instrumentType) {
        ...
        this.extraRate = 1.0;
        if (instrumentType === 'soprano-recorder') this.extraRate = 2.0;
        if (instrumentType === 'tenor-recorder') this.extraRate = 0.5; 
        if (instrumentType === 'bass-recorder') this.extraRate = 0.25;
        this.sampleMap = {
            "C4": "C4.mp3",
            "E4": "E4.mp3",
            "A4": "A4.mp3",
    ...
    start({ note, time, duration }) {
        ...
        for (let s in noteToMidi) {
            let diff = Math.abs(note - noteToMidi[s]);
            if (diff < minDiff) {
                minDiff = diff;
                nearest = s;
            }
        ...
        const rate = Math.pow(2, (note - sampleMidi) / 12);

        const source = this.ac.createBufferSource();
        source.buffer = buffer;
        source.playbackRate.value = rate * this.extraRate;

        const gainNode = this.ac.createGain();
        gainNode.gain.setValueAtTime(1, time);
        let fadeOutStart = Math.max(time, time + duration - 0.05);
